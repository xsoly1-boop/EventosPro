"use client";
// TEMA 2 — Split Hero: imagen izquierda + filas horizontales derecha (diseño actual)
  ...
  return (
    <div className="flex min-h-screen bg-obsidian">
      ...
      <RightPanel>
        ...
        <div className="w-14 h-14 rounded-2xl bg-gold/10 border border-gold/20 flex items-center justify-center mb-5"><Icon className="text-gold h-6 w-6" /></div>
        <p className="text-[10px] text-gold/60 tracking-[0.25em] uppercase font-semibold mb-1.5">{eyebrow}</p>
        <h2 className="text-2xl font-semibold text-white mb-1">{title}</h2>
        <p className="text-gray-500 text-xs font-light mb-8">Ingresa tus credenciales para continuar.</p>
        {children}
      </RightPanel>
    </div>
  );
    ...
    <div className="flex min-h-screen bg-obsidian">
      ...
      <RightPanel>
          ...
          {ACCESS_ROWS.map((row, i) => {
            ...
            return (
              <button key={row.id} onClick={() => handleRowAction(row.action)}
                ...
                <div className={`flex items-center gap-4 rounded-2xl px-5 py-4 transition-colors duration-300 ${row.featured ? "bg-gradient-to-r from-gold/[0.12] to-amber-400/[0.06] group-hover:from-gold/[0.18]" : "bg-white/[0.03] group-hover:bg-white/[0.06]"}`}>
                  <div className={`shrink-0 w-11 h-11 rounded-xl flex items-center justify-center transition-all duration-300 ${row.featured ? "bg-gold/20 border border-gold/30 group-hover:bg-gold/30" : "bg-white/5 border border-white/10 group-hover:border-gold/20 group-hover:bg-gold/10"}`}>
                    <Icon className={`h-4 w-4 transition-colors duration-300 ${row.featured ? "text-gold" : "text-gray-400 group-hover:text-gold"}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className={`text-[9px] font-semibold tracking-[0.2em] uppercase ${row.featured ? "text-gold/70" : "text-gray-500"}`}>{row.eyebrow}</span>
                      {row.featured && <span className="text-[8px] font-bold uppercase tracking-widest text-obsidian bg-gold px-1.5 py-0.5 rounded-full">Popular</span>}
                    </div>
                    <p className={`text-sm font-semibold transition-colors duration-300 ${row.featured ? "text-white" : "text-gray-200 group-hover:text-white"}`}>{row.title}</p>
                    <p className="text-gray-500 text-[11px] font-light mt-0.5 leading-relaxed line-clamp-1">{row.desc}</p>
                  </div>
                  <div className={`shrink-0 w-8 h-8 rounded-xl flex items-center justify-center transition-all duration-300 ${row.featured ? "bg-gold text-obsidian shadow-[0_2px_12px_rgba(212,175,55,0.4)] group-hover:scale-110" : "bg-white/5 text-gray-400 group-hover:bg-gold group-hover:text-obsidian group-hover:scale-110"}`}>
        ...
        <div className="mt-10 pt-6 border-t border-white/5 flex items-center justify-between">
          ...
          <div className="flex gap-1">{[0,1,2].map(i=><div key={i} className="w-1 h-1 rounded-full bg-gold/30" style={{animation:`pulse 2s ease ${i*0.3}s infinite`}} />)}</div>
        </div>
      </RightPanel>
    </div>
  );
  ...
  if (activeForm === "host-code") return (
    <FormShell title="Ingresar Código" eyebrow="Portal Anfitrión" icon={KeyRound} setActiveForm={setActiveForm}>
      <form onSubmit={handleHostSubmit} className="space-y-4">
        ...
        {error && <div className="flex items-center gap-2 text-red-400 text-xs bg-red-950/30 border border-red-500/15 rounded-xl px-4 py-3"><AlertCircle className="h-3.5 w-3.5 shrink-0" /><span>{error}</span></div>}
        <button type="submit" disabled={loading} className="w-full py-3.5 rounded-xl bg-gradient-to-r from-gold to-amber-400 text-obsidian text-xs font-bold uppercase tracking-wider transition-all shadow-[0_4px_20px_rgba(212,175,55,0.25)] hover:shadow-[0_6px_30px_rgba(212,175,55,0.45)] disabled:opacity-50 flex items-center justify-center gap-2">
          {loading ? <><div className="w-3.5 h-3.5 border-2 border-obsidian border-t-transparent rounded-full animate-spin" /><span>Verificando...</span></> : <><span>Acceder al Panel</span><ArrowRight className="h-3.5 w-3.5" /></>}
        </button>
      </form>
    </FormShell>
  );
